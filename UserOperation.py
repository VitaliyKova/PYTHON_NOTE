...
class UserOperation(object):
    ...
    def editBody(self, id, newValue):
        for note in self.new_book.new_list:
            if note.id == id:
                note.body = newValue # Перезаписывается поле заголовок с новым значением
                print("\nУспешно сохранено!\n")
            self.new_book.recordingToFile()# Перезапись файла
    ...
    def date_search(self, searchDate):
        flag = False
        for note in self.new_book.new_list:   
            if note.date.strftime("%d-%m-%Y") == searchDate:  
                print(f"\n{note.date}\nID: {note.id}\nTitle: {note.title}\nBody: {note.body}\n\n")
                flag = True
                continue
            else: continue           
